...
description: Разбор тикетов MDBSUP и инцидентов INCALL («инцы», автоинциденты из Grafana OnCall) про произвольные проблемы с кластерами MDB — зависшие/упавшие операции, недоступность, ошибки подключения/авторизации клиентов, конфиги/PMS, миграции, resize. Диагностика по цепочке Jira/алерт → прод-БД (db-worker) → прод-Temporal (temporal-worker) → mcc-хосты (mcc-host-worker); кластерная специфика — в скиллах-инспекторах (Kafka/Redis/Mongo/ClickHouse/PostgreSQL). Статус/закрытие инцидента INCALL — командами /s и /close в беседе myteam, не в Jira. План правок и исполнение ТОЛЬКО после явного одобрения пользователя. Используй при запросах вида «разбери MDBSUP-XXXX», «разберись с инцом I52118», «зависшая операция», «найди кластер и операции в темпорал».
allowed-tools: [bash]
---
...
**Не лезть в код сервисов** (mdb-processing, mdb-backend, one-cloud-ops и др. репо) по
собственной инициативе. Чтение исходников — только по явному запросу пользователя или
после его подтверждения, и всегда с обоснованием, зачем это нужно для диагноза (например:
stackTrace без кода непонятен, нужно сверить фактическую retry-политику). По умолчанию
диагностика исчерпывается цепочкой ниже (Jira → БД → Temporal → mcc) + каталогами
проблем в скиллах-инспекторах и их `history/`.

## Инциденты INCALL (алерты, «инцы»)

Помимо MDBSUP бывают **автоинциденты из Grafana OnCall** — тикеты `INCALL-<NNNNN>`;
в разговоре их называют коротко «инц» / «I52118» (это INCALL-52118). У инцидента
4 сущности: Flare (`flare.vk.team/incidents/<id>`), alert-group Grafana OnCall
(`goc.vk.team/.../alert-groups/<ID>`), Jira-тикет `INCALL-<key>` и **беседа в myteam**
(заголовок вида `[P3] I<NNNNN>: <текст алерта> [O]`, dialog sn вида `176012169330689@chat.agent`).

Порядок:

1. **Найти беседу и контекст**: `vkws_search_messages` по имени кластера/хоста из
   описания, scope=all — в выдаче алерт-посты OnCall Bot'а и сама беседа инцидента.
   Из алерта достать: хост, cluster_id, severity, ссылку на инструкцию (Confluence).
   Jira-тикет INCALL только дублирует текст алерта — отдельно его разбирать не нужно.
2. **Диагностика** — как для MDBSUP: хост/кластер из алерта → mcc → скилл-инспектор
   типа БД (шаги 4–5 «Общего порядка»).
3. **Статус и закрытие — ТОЛЬКО в беседе, в Jira-тикет НЕ писать.** Обычными сообщениями
   в тред инцидента отправить `/s` + краткий статус (что случилось, что с кластером
   сейчас, импакт), следом отдельным сообщением `/close`. Стиль текста — text-writer.
   Формат `/s`: статусный текст писать В ТОЙ ЖЕ СТРОКЕ после `/s` через один пробел
   (`/s <текст статуса>`), НЕ переносить его на следующую строку и не оформлять
   многострочным сообщением.
   ⚠️ В Jira-тикет INCALL ничего писать не нужно (в том числе `/s`, `/close` и статусные
   комментарии): команды в тикете не работают, тикет — зеркало алерта. Проверено на
   INCALL-52118: `/s` и `/close`, отправленные в Jira, статус не изменили; инцидент
   закрылся только после тех же команд в беседе.
...
6. **Закрытие — всегда ДВА комментария** (см. «Комментарий для Developers» ниже):
    публичный заявителю + технический разбор с visibility role=Developers (REST curl).
    Оба — в том же ходе, что и закрытие тикета; НЕ спрашивать пользователя.
...
## Комментарий для Developers — по умолчанию, без уточнения

На каждый разобранный MDBSUP (диагностика и/или фикс выполнены) пишется **два** комментария:

1. **Публичный** — заявителю: результат + следующий шаг (text-writer, раздел Jira).
2. **Для Developers** — технический разбор «как диагностировали и как чинили», видимость
   только роль **Developers**. Формат и доставка — text-writer, раздел «Jira (комментарий
   для Developers)»: CAPS-секции `ПРОБЛЕМА / ДИАГНОСТИКА / РЕШЕНИЕ / ПРОВЕРКА / ОСТАТОК`,
   нумерованные шаги с командами и подводными камнями; MCP-инструмент visibility не
   поддерживает — постить через REST curl с `{"visibility": {"type": "role", "value":
   "Developers"}}`. Назначение — датасет для обучения бота.

Писать его **автоматически, в том же ходе работ**, что и публичный комментарий —
**не спрашивать пользователя**. Дополнительного одобрения не требует: одобрение плана
фикса покрывает и этот комментарий. Не писать только если пользователь явно сказал
«без разбора для Developers».
...
### Шаг 0 (обязательно): статус операции в прод-БД — до любого глубокого копания

```sql
SELECT id, status, attempts_left, created_ts, started_ts, finished_ts,
       left(coalesce(error_message,''),200) AS err
FROM operations WHERE id = '<operationId>';
```

- **`done`** → глубоко НЕ копать. Достаточно подтвердить, что операция реально
  завершилась успехом: в Temporal последний run workflowId=operationId —
  `WORKFLOW_EXECUTION_STATUS_COMPLETED` (listing-запрос, скилл `temporal-worker`),
  либо, если Temporal пуст, — в операторе one-cloud-ops (задача исчезла из tasks).
  Дальше: комментарий в тикет (text-writer) и закрытие. Полный разбор истории
  workflow нужен только если статус done вызывает сомнения.
- Тикет мог быть создан ДО завершения (L1/бот фиксируют «зависло» по RUNNING);
  операция часто сходится сама за десятки минут — сначала проверяем актуальный
  статус, потом копаем (пример: MDBSUP-4938 — сошлась через 34 мин после тикета).
- Не `done` (in_progress/error + in_processing) → полный сценарий ниже.

- workflowId операции = operationId; упавший child/activity искать рекурсией cause
  (скилл `temporal-worker`).
- ⚠️ Сначала «Шаг 0» выше — статус операции в прод-БД; при `done` глубокий разбор
  не нужен.
...
- **INCALL: `/s` и `/close` — только в беседу инцидента (myteam), не в Jira-тикет.**
  В тикете команды не срабатывают (INCALL-52118); писать туда статусные комментарии
  не нужно — тикет лишь зеркало алерта.
...
- **Комментарии/ответы в тикетах** — текст писать через скилл **`text-writer`**
  (раздел Jira): кратко, результат + следующий шаг, без перечня действий. На каждый
  разобранный MDBSUP — также разбор для Developers по умолчанию (см. одноимённый
  раздел выше), уточнять пользователя не нужно.
- **Любые reassign партиций Kafka** (перемещение, вывод брокера, изменение RF/replicas) —
  **только через скилл `kafka-reassign-partitions`**: никакой самописной Java с
  alterPartitionReassignments и ручных правок assignment в обход него. Перед reassign
  проверять, не исполняет ли сейчас Cruise Control свои движения по этим партициям
  (`GET /kafkacruisecontrol/state?substates=executor`), иначе конфликт исполнения.
