...
## 2026-09-11: конфиг-фикс применён на reactions-prod-vk-reactions-kafka (прод, ns vk-reactions)

Контекст: разбор MDBSUP-3603 — брокеры упираются в пресет c.large=16 vCPU (облачный CPU
100% на 2.zc/2.ic), CC при этом видит ~0% и рапортует RIGHT_SIZED. PCL на кластере
полумёртвый (2.zc константный 0.0, ic ~0.005). Несмотря на это, пользователь одобрил
применение варианта A (capacity-фикс).

Сделано:
1. Z миньонов кластера: nc=128, zc=128, ic=112 (grep -c processor /proc/cpuinfo с брокера).
2. PMS `kafka.cruisecontrol.capacity.json` (ключ `reactions-prod-vk-reactions-kafka.clouds`,
   cruise делит ключ с брокерами): `"CPU": "100"` → `"12.5"` (16×100/128), DISK/NW не тронуты.
   update.do → верификация байт-в-байт.
3. Cruise-хост: `confp --oneshot` (файл обновился) + `systemctl restart cruise-control`.
4. Верификация через `/kafkacruisecontrol/load?json=true`: **NumCore=0.125** (маркер
   применения строковой capacity), **CpuPct 9.0% (20001) / 16.4% (20002)** — не нули.
   isProposalReady=true через ~3 мин после рестарта.

Нюансы:
- ic-брокеры (20001/20002): capacity завышена ~12.5% (Z=112 vs 128) — при желании
  добавить per-broker записи `"CPU": "14.29"`.
- Если PCL снова уйдёт в 0.0 — CpuPct обнулится, CPU-цели ослепнут (известное поведение).
- modify-флоу затрёт значение шаблоном → доделать шаблон в backstage (MR !234).

### CC vs реальность после фикса (срез 11.09, /load)

| Брокер | CC CpuPct | Реально (облако) |
|---|---|---|
| 20001 (1.ic) | 8.5% | ~49% |
| 20002 (2.ic) | 16.5% | 100% |
| 21001 (1.nc) | **0.23%** | 67% |
...
Картина **инвертирована**: CC считает самыми загруженными ic (PCL там дышит), а nc/zc —
простаивающими (PCL константный 0), в реальности наоборот. Подтверждён вывод msk-эксперимента:
конфиг-фикс даёт правильную шкалу, но не чинит метрику; на кластерах с мёртвым PCL
CPU-цели CC непригодны. По MDBSUP-3603 изменение **откачено** в тот же день (см. ниже),
целевой фикс — вариант B (патч репортёра).

**Откат 11.09 (по решению пользователя):** PMS CPU `"12.5"` → `"100"` (байт-в-байт исходник),
confp --oneshot, рестарт CC, верификация файла.
