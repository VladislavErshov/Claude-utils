---
name: db-seed
description: Используй этот скилл, когда нужно заполнить локальные таблицы данными из удалённой (продовой) базы для тестирования API. Работает по схеме: пользователь указывает что копировать → скилл сам читает удалённую БД через read-only туннель localhost:53480 (пользователь не нужен) → вставляет данные в локальную БД. Если туннель не поднят — генерирует SQL для удалённой БД, пользователь выполняет их и отдаёт результат.
allowed-tools: [bash]
---
...
## Прямое подключение к продовой БД (read-only)

Подключайся к продовой БД напрямую сам — **разрешение пользователя не нужно, но только SELECT (read-only)**. Никаких INSERT/UPDATE/DELETE/DRAFT-изменений на проде.

- Хост: `localhost:53480` (туннель до прода)
- БД: `backstage_plugin_mdb`, пользователь: `backstage`, пароль: `HDX!cpw5yxf0ypd5tgd`
- Локально `psql` отсутствует — клиент бери из контейнера `pg_backstage_plugin_mdb`, хост внутри docker — `host.docker.internal`:
...
Это заменяет ручной шаг «пользователь выполняет SQL через mcc ssh и отдаёт результат»: сгенерированный `jsonb_build_object`-запрос выполняй сам через это подключение (вывод в файл `> /tmp/<cluster>.json`). Ручной путь через `mcc ssh` — запасной, если туннель на 53480 не поднят.
...
3. **Генерация SQL для удалённой БД** — написать SELECT-запрос. **Всегда одним запросом** через `jsonb_build_object` (см. шаблон ниже). Если туннель на `localhost:53480` доступен — выполни его сам (read-only, см. выше); иначе пользователь выполнит его на удалённом хосте (через скилл [`mcc-host-worker`](../mcc-host-worker/SKILL.md), `mcc ssh` + `psql`).
