...
⚠️ **Назначение — только БД** (postgres и другие СУБД, куда иначе с ноутбука не попасть).
Для выполнения команд на хостах телепорт НЕ использовать — есть `mcc ssh`/`sshexec`
(см. [sshexec.md](sshexec.md)). Схемы вида «туннель к сервисному порту + /etc/hosts
на 127.0.0.1» — антипаттерн: AAAA-резолв обходит hosts, один локальный порт не
обслуживает несколько FQDN, правка /etc/hosts требует sudo и ломает остальные кластеры.
