...
2. **UUID из ссылки UI → алиас кластера**: `db_cluster` даёт короткое имя (`events`),
   ...
   креды в db-worker). Угадывание по подстроке через `mcc instances "%front%"` даёт мусор
...
## Ревизия 2026-09-08: перепроверка после фикса

Повторный прогон `nc -zv -w5 [-4|-6] <fqdn> 9092` с брокеров pc/hc/uc обоих кластеров:

- **Внутрикластерное — ПОЧИНЕНО.** comments UC теперь принимает 9092 по v4 от своих
  pc и hc (главная аномалия 2026-09-07 закрыта). events — по-прежнему полностью OK.
- **Межкластерное PC/HC→UC — не изменилось, и теперь видна симметрия**: фильтруется
  любой межкластерный трафик В СТОРОНУ UC: events pc/hc → comments UC ❌ (v4+v6) и
  comments pc → events UC ❌ (v4+v6). UC как источник — всё ✅. Остальные
  межкластерные направления (→pc, →hc) — ✅.
- Вывод: остаточное правило — NSG-политика периметра UC «межкластерный вход извне
  кластера в UC запрещён», не бага конкретного хоста. Внутрикластерная репликация
  (реальная потребность кейса) работает на обоих кластерах.
